      ...
      <div className="flex-1 p-2 overflow-hidden flex flex-col relative bg-white">
        <AnimatePresence mode="wait">
          ...
          {step === 0 && (
            ...
            >
              ...
              >
                ...
                <div>
                  <h3 className="text-lg font-bold text-teal-900">Engagement Model</h3>
                  <p className="text-teal-700 text-sm mt-1 leading-relaxed">How deep is the partnership?</p>
                </div>
              ...
              <div className="grid grid-cols-1 gap-3">
                {[
                  ...
                  >
                    <div className={`p-2 rounded-lg shrink-0 ${involvement === m.id ? 'bg-teal-600 text-white' : 'bg-gray-100/50'}`}>
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs opacity-80 leading-snug mt-1">{m.desc}</p>
                    </div>
                  </button>
                ))}
              </div>
          ...
          {step === 1 && (
            ...
            >
              <div className="text-center mb-6">
                <h3 className="text-lg font-bold text-gray-900">Terms & Governance</h3>
                <p className="text-sm text-gray-500">Timeline and decision making</p>
              </div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {['6 Months', '1 Year', '2 Years', '3+ Years'].map(d => (
                    <button
                      ...
                      className={`py-3 rounded-lg border text-xs font-bold transition-all ${duration === d ? 'bg-teal-600 border-teal-600 text-white' : 'bg-white border-gray-100 text-gray-400 hover:border-gray-300'}`}
                    >
                      {d}
                    </button>
                  ))}
              ...
              <div className="space-y-4">
                <label className="block text-sm font-bold text-gray-700 uppercase tracking-widest">Decision Authority</label>
                <div className="flex bg-gray-50 p-1.5 rounded-xl">
                  {['GCC Owned', 'Shared', 'Partner Owned'].map(a => (
                    <button
                      key={a}
                      onClick={() => onUpdate({ authority: a })}
                      className={`flex-1 py-3 rounded-lg text-xs font-bold transition-all ${authority === a ? 'bg-white text-teal-800 shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}
                    >
                      {a}
                    </button>
                  ))}
                ...
                <div className="flex gap-2">
                  {['Weekly', 'Bi-Weekly', 'Monthly'].map(c => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-lg border text-[10px] font-bold ${cadence === c ? 'bg-teal-50 border-teal-500 text-teal-700' : 'bg-white border-gray-100 text-gray-400'}`}
                    >
                      {c}
                    </button>
                  ))}
          ...
          {step === 2 && (
            ...
            >
              <div className="text-center mb-6">
                <h3 className="text-lg font-bold text-gray-900">Scope & Outcomes</h3>
                <p className="text-sm text-gray-500">Deliverables and success factors</p>
              </div>
                    ...
                    {SERVICES.map(s => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left px-4 py-3 rounded-xl border text-xs font-bold transition-all ${active ? 'bg-teal-50 border-teal-500 text-teal-800' : 'bg-white border-gray-100 text-gray-400 hover:border-gray-200'}`}
                        >
                          {s}
                        </button>
                      );
                    ...
                    {['Faster Time-to-Market', 'Cost Reduction > 30%', 'Process Standardization', 'Access to Niche Talent'].map(o => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full flex items-center justify-between p-3 rounded-xl border transition-all ${active ? 'bg-indigo-50 border-indigo-200' : 'bg-white border-gray-100'}`}
                        >
                          <span className={`text-xs font-bold ${active ? 'text-indigo-800' : 'text-gray-400'}`}>{o}</span>
