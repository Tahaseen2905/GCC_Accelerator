...
export const QuestMission: React.FC<QuestMissionProps> = ({ industry, companyName, goals, maturity, onUpdate, onNext, onPrev }) => {
  ...
  return (
    ...
    >
      <div className="flex-1 flex flex-col justify-center gap-3 p-1 max-w-lg mx-auto w-full">
        <motion.div
          initial={{ x: -20, opacity: 0 }}
          animate={{ x: 0, opacity: 1 }}
          transition={{ delay: 0.1 }}
          className="bg-gradient-to-br from-teal-50 to-white border border-teal-100 rounded-2xl p-3 flex items-center gap-4 shrink-0 shadow-[4px_4px_10px_rgba(13,148,136,0.05)] max-w-lg mx-auto w-full"
        >
          ...
          <div>
            <h3 className="text-base font-bold text-teal-900">Define Your Mission</h3>
            <p className="text-teal-700 text-xs mt-0.5 leading-relaxed">Set the foundational scope for your Global Capability Center.</p>
          </div>
        ...
        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4 items-center content-center">
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              className="w-full bg-white border border-gray-100 rounded-xl px-3 py-2.5 text-xs text-gray-700 font-medium focus:border-teal-500 outline-none transition-all shadow-sm placeholder-gray-400"
            />
          </div>

          <div className="space-y-2">
            <label className="block text-xs font-bold text-gray-700 uppercase tracking-wider">Industry Sector</label>
            <div className="relative">
              <select
                value={industry}
                onChange={(e) => onUpdate({ industry: e.target.value })}
                className="w-full bg-white border border-gray-100 rounded-xl px-3 py-2.5 text-xs text-gray-700 font-medium focus:border-teal-500 outline-none transition-all appearance-none cursor-pointer shadow-sm"
              >
          ...
          <div className="space-y-2 md:col-span-2">
            <label className="block text-xs font-bold text-gray-700 uppercase tracking-wider">Digital Maturity</label>
            <div className="grid grid-cols-3 gap-3">
              {(['Ad-hoc', 'Defined', 'Optimized'] as DigitalMaturity[]).map(m => (
                <button
                  key={m}
                  onClick={() => onUpdate({ maturity: m })}
                  className={`py-2 px-2 rounded-xl border font-bold text-xs transition-all ${maturity === m ? 'border-teal-500 bg-teal-50 text-teal-700' : 'border-gray-100 text-gray-400 hover:border-gray-200'}`}
                >
                  {m}
                </button>
              ))}
        ...
        <div className="space-y-2 shrink-0">
          <label className="block text-xs font-bold text-gray-700 uppercase tracking-wider">Primary Goals (Multi-select)</label>
            ...
            {PRIMARY_GOALS.map(goal => {
              ...
              return (
                <motion.button
                  ...
                  className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-full border font-semibold text-[11px] transition-all ${isActive
